"""TTS facade: Cartesia → ElevenLabs → silent track.
Returns (path, source, word_timings). word_timings is [] for providers that
don't return timing (only Cartesia does today)."""
import asyncio
...
async def synthesize(
    ...
    if settings.has_cartesia:
        try:
            ...
            _, word_timings = await cartesia_synthesize(text, out_path, language)
            return out_path, "cartesia", word_timings
